...
resource "yandex_iam_service_account" "admin01" {
  name = "admin01"
}
...
resource "yandex_resourcemanager_folder_iam_binding" "editor" {
  ...
  role = "editor"
  members = [
    "serviceAccount:${yandex_iam_service_account.admin01.id}",
  ]
  depends_on = [
    yandex_iam_service_account.admin01,
  ]
}

resource "yandex_iam_service_account_static_access_key" "static-access-key" {
  service_account_id = yandex_iam_service_account.admin01.id
  depends_on = [
    yandex_iam_service_account.admin01,
  ]
}
...
resource "yandex_compute_instance_group" "k8s-masters" {
  ...
  instance_template {
   ...
   # metadata = {
   #   ssh-keys = "ubuntu:${file("home/.ssh/id_rsa.pub")}"
    #}
    network_settings {
      type = "STANDARD"
    }
